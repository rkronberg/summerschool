program datatype_struct
  ...
  call mpi_type_create_struct(cnt,blocklen,disp,types,mpi_byte,ierror)
  call mpi_type_commit(mpi_byte,ierror)
  ...
  if(myid == 0) then
     do i = 1, 1000
        call MPI_SEND(particles, n, mpi_byte, 1, i, &
             & MPI_COMM_WORLD, ierror)
     end do
  else if(myid == 1) then
     do i = 1, 1000
        call MPI_RECV(particles, n, mpi_byte, 0, i, &
  ...
  call MPI_TYPE_free(mpi_byte, ierror)
